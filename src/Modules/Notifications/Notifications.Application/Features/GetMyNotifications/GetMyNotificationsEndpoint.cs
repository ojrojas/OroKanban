...
public sealed class GetMyNotificationsEndpoint : IEndpoint
{
    ...
    {
        app.MapGet("/api/notifications", async (HttpContext ctx, ISender sender, int? page, int? pageSize, bool? unreadOnly, int? typeId, CancellationToken ct) =>
        {
            ...
            var query = new GetMyNotificationsQuery(callerId, tenantId, page ?? 1, pageSize ?? 20, unreadOnly ?? false, typeId);
