...
@Component({
  ...
  template: `
    ...
    <div class="filter-pills" style="margin-bottom: var(--gap-widget); display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
      <button class="pill" [class.active]="store.filter()==='all'" (click)="store.setFilter('all'); store.load()">All</button>
      <button class="pill" [class.active]="store.filter()==='Created'" (click)="store.setFilter('Created'); store.load()">Created</button>
      <button class="pill" [class.active]="store.filter()==='Updated'" (click)="store.setFilter('Updated'); store.load()">Updated</button>
    ...
    @else {
      <div class="tier-2" style="padding:0; overflow:hidden;">
        <div style="padding:16px 24px; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;"><h3 style="margin:0; font-size:14px; font-weight:700;">Audit entries</h3><span class="badge">{{store.filtered().length}}/{{store.count()}}</span></div>
        @for (a of store.filtered(); track a.id || a.timestamp) {
          <div class="row">
            <div class="thumb"></div>
