...
public sealed class ManagementRelationshipConfiguration : IEntityTypeConfiguration<ManagementRelationship>
{
    public void Configure(EntityTypeBuilder<ManagementRelationship> builder)
    {
        ...
        builder.Property(x => x.Id).HasConversion(id => id.Value, v => new Organization.Domain.ValueObjects.ManagementRelationshipId(v));
        builder.Property(x => x.TenantId).IsRequired();
        builder.Property(x => x.ManagerId).IsRequired();
        builder.Property(x => x.SubordinateId).IsRequired();
        builder.Property(x => x.Type).IsRequired().HasMaxLength(50);
        builder.Property(x => x.OrganizationUnitId).HasConversion(id => id != null ? id.Value : (Guid?)null, v => v != null ? new Organization.Domain.ValueObjects.OrganizationUnitId(v.Value) : null);
