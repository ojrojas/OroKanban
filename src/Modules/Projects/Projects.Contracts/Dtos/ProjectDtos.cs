...
public sealed record WorkItemDetailResponse(Guid Id, Guid ProjectId, Guid? ParentId, string Title, string? Description, string Type, string Status, string Priority, string Criticality, Guid? OwnerId, Guid? ResponsibleId, Guid? ReviewerId, DateTime? DueDate, int Progress, IReadOnlyList<string> Tags, IReadOnlyList<string> Deliverables, string? Observations, int Version, DateTime UpdatedAt, Guid TenantId, bool IsOverdue, IReadOnlyList<Guid> DependencyIds, decimal EstimatedHours, decimal ActualHours, DateTime? StartedAt, int ReopenedCount, bool CanEdit);
public sealed record DeliverableDto(Guid Id, Guid WorkItemId, string Title, string Type, string Status, string? Url, DateTime CreatedAt);
public sealed record WorkItemHistoryDto(Guid Id, Guid WorkItemId, string Field, string? FromJson, string? ToJson, string? Comment, DateTime CreatedAt, Guid? ActorId);
...
public sealed record BoardItemDto(Guid Id, string Title, string Type, string Status, string Priority, string Criticality, Guid? ResponsibleId, DateTime? DueDate, bool IsOverdue, int Progress, IReadOnlyList<string> Tags, Guid? ParentId, Guid? EpicId, bool BlockedDerived, int Version, DateTime UpdatedAt, decimal EstimatedHours, decimal ActualHours);
